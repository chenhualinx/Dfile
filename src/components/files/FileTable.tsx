...
export function FileTable({ entries, viewMode, selected, onSelectionChange, onNavigate, onDownload, onDelete }: FileTableProps) {
  const parentRef = useRef<HTMLDivElement>(null)

  const flatEntries = useMemo(
    () => entries.filter((e) => e.is_directory).concat(entries.filter((e) => !e.is_directory)),
    [entries],
  )
